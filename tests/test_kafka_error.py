...
fake_ck.ConsumerGroupTopicPartitions = type("ConsumerGroupTopicPartitions", (), {"__init__": lambda self, gid: None})
...
def test_analyze_kafka_cluster_handles_adminclient_failure(monkeypatch):
    ...
    assert any(f.get("severity") == "ERROR" for f in findings), "Expected an ERROR finding when connection fails"
